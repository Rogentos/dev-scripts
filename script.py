...
filename="equo.db"
new_filename="equo_backup.db"
path="/var/lib/entropy/client/database/"
processor = ""
...
def rename(var):
    if ( os.path.isfile(var+filename)==True ):
        print "fine"
        os.rename(var+filename,var+new_filename)
    else:
        print "The given file "+var+filename+" does not exist"

def create_new_file(var):
    with open(var+filename,'a'):
        os.utime(var+filename,None)

def permissions(var):
    subprocess.call("chgrp entropy "+var+filename, shell=True)
    subprocess.call("ls -l "+var+filename,shell=True)  

def insert_sql(var):
    conn = sqlite3.connect(var+filename)
    c = conn.cursor()
    ...
    INSERT INTO \"settings\" VALUES('arch','x86');
    INSERT INTO \"settings\" VALUES('on_delete_cascade', '1');
    INSERT INTO \"settings\" VALUES('_baseinfo_extrainfo_2010', '1');
    INSERT INTO \"settings\" VALUES('schema_revision', '6');
    COMMIT;""")
...
def rescue():
    subprocess.call("equo rescue spmsync",shell=True)
